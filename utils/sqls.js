 ...
 *************************************************************/
//(1) 查询表中所有数据
var QUERY_DATAS = (tableName, wheres) => {
  if (!wheres) return `SELECT * FROM ${tableName}`
  let whe = []
  for (var key in wheres) {
    whe.push(`${key}=${wheres[key]}`)
  }
  return `SELECT * FROM ${tableName} where ` + whe.join(' and ')
};
//(2) 插入数据(全部列)
var INSERT_DATAS = (tableName, values) => {
    return `INSERT INTO ${tableName} ( ${Object.keys(values).join(',')}) VALUES ('${Object.keys(values).map(key=> values[key]).join("','")}');`;
}
...
//(4) 删除数据(根据id)
var DELETE_DATA_BY_OPTIONS = (tableName, wheres) => {
  let whe = []
  for (var key in wheres) {
    whe.push(`${key}=${wheres[key]}`)
  }
  return `DELETE FROM ${tableName} WHERE ` + whe.join(' and ');
}
  
//(5) 删除所有数据
var DELETE_DATAS = (tableName) => `DELETE FROM ${tableName};`;
//(6) 更新数据条目
var UPDATE_DATA = (tableName, id, wheres) => {
  let whe = []
  for (var key in wheres) {
    whe.push(`${key}='${wheres[key]}'`)
  }
  return `UPDATE ${tableName} SET  ${whe.join(', ')} WHERE id = ${id};`;
}
